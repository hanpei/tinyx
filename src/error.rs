...
impl Error {
    ...
    pub fn parse_number_error(file: &str, pos: Pos) -> Error {
        Self::ParseError(format!("invalid number at {}:{}:{}", file, pos.ln, pos.col+1))
    }
    pub fn invalid_token(file: &str, pos: Pos) -> Error {
        Self::InvalidToken(format!("invalid token at {}:{}:{}", file, pos.ln, pos.col+1))
    }
